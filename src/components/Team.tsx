...
export const Team = ({ onClick }: Props) => {
  const { team, removeFromTeam } = useStore();
  const hasAnyone = team.some((p) => p !== null);

  return (
    <div className="flex gap-4 flex-col items-center">
      <div className="flex items-center gap-2">
        <h2 className="font-bold text-sm uppercase">Mitt lag</h2>
        {hasAnyone && (
          <span className="text-slate-500 text-xs">
            ({team.filter(Boolean).length}/5)
          </span>
        )}
      </div>
